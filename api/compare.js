...
- If the best choice depends on preferences not provided, use "No clear winner" with confidence "low"
...
`.trim();
  ...
  // IMPORTANT: wrap everything in .comparison and include data-options for the "Visit official site" UI
  let html = `<div class="comparison" data-options='${escapeHtml(
    JSON.stringify(options.map(o => ({ name: o.name, url: o.url })))
  )}'>`;

  // Styles (kept inline so the output is always readable)
  html += `
  <style>
  ...
  html += `</div>`; // close .comparison wrapper
